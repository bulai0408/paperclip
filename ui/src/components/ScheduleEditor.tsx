...
function describeSchedule(cron: string, t?: (key: string, opts?: Record<string, string>) => string): string {
  ...
  if (!t) {
    ...
    switch (preset) {
      ...
      case "weekly": {
        const dayKey = DAYS_OF_WEEK.find((d) => d.value === dayOfWeek)?.labelKey ?? dayOfWeek;
        return `Every ${dayKey} at ${timeStr}`;
      }
      case "monthly":
  ...
  switch (preset) {
    ...
    case "every_hour":
      return t("schedule.everyHourAt", { minute: minute.padStart(2, "0") });
    case "every_day":
      return t("schedule.everyDayAt", { time: timeStr });
    case "weekdays":
      return t("schedule.weekdaysAt", { time: timeStr });
    case "weekly": {
      const dayKey = DAYS_OF_WEEK.find((d) => d.value === dayOfWeek)?.labelKey ?? dayOfWeek;
      return t("schedule.weeklyAt", { day: t(`schedule.${dayKey}`), time: timeStr });
    }
    case "monthly":
      return t("schedule.monthlyAt", { day: `${dayOfMonth}${ordinalSuffix(Number(dayOfMonth))}`, time: timeStr });
    case "custom":
