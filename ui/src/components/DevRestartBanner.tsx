...
function formatRelativeTimestamp(value: string | null, t: (key: string) => string): string | null {
  ...
  if (deltaMinutes < 60) return t("devRestartBanner.minutesAgo").replace("{{count}}", String(deltaMinutes));
  const deltaHours = Math.round(deltaMinutes / 60);
  if (deltaHours < 24) return t("devRestartBanner.hoursAgo").replace("{{count}}", String(deltaHours));
  const deltaDays = Math.round(deltaHours / 24);
  return t("devRestartBanner.daysAgo").replace("{{count}}", String(deltaDays));
}

function describeReason(devServer: DevServerHealthStatus, t: (key: string) => string): string {
  if (devServer.reason === "backend_changes_and_pending_migrations") {
    return t("devRestartBanner.reasonBackendAndMigrations");
  }
  if (devServer.reason === "pending_migrations") {
    return t("devRestartBanner.reasonPendingMigrations");
  }
  return t("devRestartBanner.reasonBackendChanged");
}
        ...
        <div className="flex shrink-0 items-center gap-2 text-xs font-medium">
          {devServer.waitingForIdle ? (
            <div className="inline-flex items-center gap-2 rounded-full bg-amber-900/10 px-3 py-1.5 dark:bg-amber-100/10">
              ...
              <span>
                {t("devRestartBanner.waitingForRuns")
                  .replace("{{count}}", String(devServer.activeRunCount))
                  .replace("{{plural}}", devServer.activeRunCount === 1 ? "" : "s")}
              </span>
            </div>
          ) : devServer.autoRestartEnabled ? (
            <div className="inline-flex items-center gap-2 rounded-full bg-amber-900/10 px-3 py-1.5 dark:bg-amber-100/10">
            ...
            <div className="inline-flex items-center gap-2 rounded-full bg-amber-900/10 px-3 py-1.5 dark:bg-amber-100/10">
              <RotateCcw className="h-3.5 w-3.5" />
              <span>{t("devRestartBanner.manualRestartHint")}</span>
            </div>
          )}
        </div>
      </div>
