...
export function ReportsToPicker({
  ...
  const resolvedDisabledEmptyLabel = disabledEmptyLabel ?? t("reportsToPicker.disabledEmptyLabel");
  const resolvedChooseLabel = chooseLabel ?? t("reportsToPicker.chooseLabel");
      ...
      <PopoverContent className="w-48 p-1" align="start">
        ...
        {unknownManager && (
          <div className="px-2 py-1.5 text-xs text-muted-foreground border-b border-border mb-0.5">
            {t("reportsToPicker.staleMissingNote")}
          </div>
        )}
        {rows.map((a) => (
          <button
