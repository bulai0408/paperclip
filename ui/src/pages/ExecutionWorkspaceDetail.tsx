...
export function ExecutionWorkspaceDetail() {
  ...
  if (workspaceQuery.isLoading) return <p className="text-sm text-muted-foreground">{t("workspace.loadingWorkspace")}</p>;
  if (workspaceQuery.error) {
    return (
      <p className="text-sm text-destructive">
        {workspaceQuery.error instanceof Error ? workspaceQuery.error.message : t("workspace.failedToLoadWorkspace")}
      </p>
    );
  }
  ...
  const saveChanges = () => {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : t("workspace.failedToBuildWorkspaceUpdate"));
      return;
    }
      ...
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_minmax(18rem,0.95fr)]">
              ...
              <div className="mt-4 grid gap-4">
                ...
                <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-3 py-3">
                  <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
                    <div>
                      <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">
                        {t("workspace.runtimeConfigSource")}
                      </div>
                      <p className="mt-1 text-sm text-muted-foreground">
                        {runtimeConfigSource === "execution_workspace"
                          ? t("workspace.runtimeConfigSourceExecWorkspace")
                          : runtimeConfigSource === "project_workspace"
                            ? t("workspace.runtimeConfigSourceProjectWorkspace")
                            : t("workspace.runtimeConfigSourceNone")}
                      </p>
                    </div>
                    <Button
                      variant="outline"
                ...
                <Field label={t("workspace.runtimeServicesJson")} hint={t("workspace.runtimeServicesJsonHint")}>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-2xl border border-border bg-card p-5">
              ...
              <DetailRow label={t("workspace.baseRef")}>
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {t("workspace.source")}: {runtimeConfigSource === "execution_workspace"
                      ? t("workspace.runtimeSourceExecWorkspace")
                      : runtimeConfigSource === "project_workspace"
                        ? t("workspace.runtimeSourceProjectWorkspace")
                        : t("workspace.runtimeSourceNone")}
                  </p>
                </div>
              ...
              {workspace.runtimeServices && workspace.runtimeServices.length > 0 ? (
                ...
                <p className="text-sm text-muted-foreground">
                  {effectiveRuntimeConfig
                    ? t("workspace.noRuntimeServicesRunning")
                    : t("workspace.noRuntimeConfigDefined")}
                </p>
              )}
            </div>
            ...
            <div className="rounded-2xl border border-border bg-card p-5">
              ...
              {workspaceOperationsQuery.isLoading ? (
                <p className="text-sm text-muted-foreground">{t("workspace.loadingWorkspaceOperations")}</p>
              ) : workspaceOperationsQuery.error ? (
                <p className="text-sm text-destructive">
                  {workspaceOperationsQuery.error instanceof Error
                    ? workspaceOperationsQuery.error.message
                    : t("workspace.failedToLoadWorkspaceOperations")}
                </p>
                ...
                <p className="text-sm text-muted-foreground">{t("workspace.noWorkspaceOperations")}</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-5">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            ...
            <StatusPill>{t("workspace.linkedCount", { count: linkedIssues.length })}</StatusPill>
          </div>
          <Separator className="my-4" />
          {linkedIssuesQuery.isLoading ? (
            <p className="text-sm text-muted-foreground">{t("workspace.loadingLinkedIssues")}</p>
          ) : linkedIssuesQuery.error ? (
            <p className="text-sm text-destructive">
              {linkedIssuesQuery.error instanceof Error
                ? linkedIssuesQuery.error.message
                : t("workspace.failedToLoadLinkedIssues")}
            </p>
            ...
            <p className="text-sm text-muted-foreground">{t("workspace.noLinkedIssues")}</p>
          )}
        </div>
      </div>
