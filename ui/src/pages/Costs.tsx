...
import { useDateRange, PRESET_KEYS, usePresetLabels } from "../hooks/useDateRange";
    ...
    <Card>
      ...
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricTile
          ...
          subtitle={t("costs.refundsDescription")}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("costs.net")}
      ...
      <div className="space-y-5">
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            ...
            <MetricTile
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("costs.budgetIncidentSubtitle", { agents: budgetData?.pausedAgentCount ?? 0, projects: budgetData?.pausedProjectCount ?? 0 })
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
            ...
            <MetricTile
              ...
              subtitle={t("costs.financeEventsSubtitle", { amount: formatCents(financeData?.summary.estimatedDebitCents ?? 0) })}
              icon={ArrowUpRight}
            />
          </div>
      </div>
                          ...
                          <div key={row.agentId} className="border border-border px-4 py-3">
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {t("costs.tokenInOut", { input: formatTokens(row.inputTokens + row.cachedInputTokens), output: formatTokens(row.outputTokens) })}
                                </div>
                                {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                  <div className="text-xs text-muted-foreground">
                                    {row.apiRunCount > 0 ? t("costs.apiRuns", { count: row.apiRunCount }) : t("costs.apiRunsZero")}
                                    {" · "}
                                    {row.subscriptionRunCount > 0
                                      ? t("costs.subscriptionRuns", { count: row.subscriptionRunCount })
                                      : t("costs.subscriptionRunsZero")}
                                  </div>
                                ) : null}
                              </div>
                            </div>
                                    ...
                                    >
                                      ...
                                      <div className="text-right tabular-nums">
                                        ...
                                        <div className="text-muted-foreground">
                                          {t("costs.tokenCount", { tokens: formatTokens(modelRow.inputTokens + modelRow.cachedInputTokens + modelRow.outputTokens) })}
                                        </div>
                                      </div>
                                    </div>
                                  );
                ...
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold capitalize">{t(`costs.scopeBudgets.${scopeType}`)}</h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? t("costs.scopeBudgetDescription.company")
                            : scopeType === "agent"
                              ? t("costs.scopeBudgetDescription.agent")
                              : t("costs.scopeBudgetDescription.project")}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
