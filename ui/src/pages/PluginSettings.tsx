...
export function PluginSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("pluginSettings.company"), href: "/dashboard" },
      { label: t("pluginSettings.settingsBreadcrumb"), href: "/instance/settings/heartbeats" },
      { label: t("pluginSettings.pluginsBreadcrumb"), href: "/instance/settings/plugins" },
                              ...
                              <>
                                <div className="flex justify-between col-span-2">
                                  ...
                                  <span className="text-xs">
                                    {dashboardData.worker.consecutiveCrashes} {t("pluginSettings.consecutive")} / {dashboardData.worker.totalCrashes} {t("pluginSettings.total")}
                                  </span>
                                </div>
                ...
                <CardContent>
                  {healthLoading ? (
                    ...
                    <div className="space-y-3 text-sm text-muted-foreground">
                      ...
                      <p>{t("pluginSettings.healthChecksRunOnceReady")}</p>
                      {plugin.lastError ? (
                        <div className="break-words rounded border border-destructive/20 bg-destructive/10 p-2 text-xs text-destructive">
                          {plugin.lastError}
                        </div>
...
function PluginConfigForm({ pluginId, schema, initialValues, isLoading, pluginStatus, supportsConfigTest }: PluginConfigFormProps) {
  ...
  const saveMutation = useMutation({
    ...
    onError: (err: Error) => {
      setSaveMessage({ type: "error", text: err.message || t("pluginSettings.failedToSaveConfiguration") });
    },
  });
