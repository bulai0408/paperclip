...
function sourceMeta(sourceBadge: CompanySkillSourceBadge, sourceLabel: string | null, t: (key: string) => string) {
  ...
  switch (sourceBadge) {
    ...
    case "local":
      return { icon: Folder, label: sourceLabel ?? t("companySkills.source.folder"), managedLabel: t("companySkills.source.folderManaged") };
    case "paperclip":
      return { icon: Paperclip, label: sourceLabel ?? "Paperclip", managedLabel: t("companySkills.source.paperclipManaged") };
    default:
      return { icon: Boxes, label: sourceLabel ?? t("companySkills.source.catalog"), managedLabel: t("companySkills.source.catalogManaged") };
  }
}
                ...
                {detail.sourcePath ? (
                  <button
                    ...
                    onClick={() => {
                      ...
                      pushToast({ title: t("skills.copiedPathToWorkspace") });
                    }}
                  >
                    {source.label}
                  </button>
...
export function CompanySkills() {
  ...
  const importSkill = useMutation({
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("skills.failedToImportSkill"),
      });
    },
  });
  ...
  const scanProjects = useMutation({
    ...
    onSuccess: async (result) => {
      setScanStatusMessage(t("skills.refreshingSkills"));
      await queryClient.invalidateQueries({ queryKey: queryKeys.companySkills.list(selectedCompanyId!) });
    ...
    onError: (error) => {
      ...
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("skills.failedToScan"),
      });
    },
  });
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Boxes} message={t("skills.selectCompanyMessage")} />;
  }
            ...
            >
              <span>
                ...
                <span className="mt-1 block text-muted-foreground">
                  {t("companySkills.findInstallCommands")}
                </span>
              </span>
          ...
          <div className="border-b border-border px-4 py-3">
            ...
            <div className="mt-3 flex items-center gap-2 border-b border-border pb-2">
              <input
                ...
                placeholder={t("skills.pastePathOrUrl")}
                className="w-full bg-transparent text-sm outline-none placeholder:text-muted-foreground"
              />
              <Button
                size="sm"
