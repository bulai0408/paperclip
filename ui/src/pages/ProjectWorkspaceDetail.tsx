...
const SOURCE_TYPE_OPTIONS: Array<{ value: ProjectWorkspaceSourceType; labelKey: string; descKey: string }> = [
  { value: "local_path", labelKey: "localGitCheckout", descKey: "localGitCheckoutDesc" },
  { value: "non_git_path", labelKey: "localNonGitPath", descKey: "localNonGitPathDesc" },
  { value: "git_repo", labelKey: "remoteGitRepo", descKey: "remoteGitRepoDesc" },
  { value: "remote_managed", labelKey: "remoteManagedWorkspace", descKey: "remoteManagedWorkspaceDesc" },
];
...
function parseRuntimeConfigJson(value: string, t: (key: string) => string) {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return {
        ok: false as const,
        error: t("workspace.runtimeServicesMustBeJson"),
      };
    }
    ...
    return {
      ok: false as const,
      error: error instanceof Error ? error.message : t("workspace.invalidJson"),
    };
  }
}
...
function validateWorkspaceForm(form: WorkspaceFormState, t: (key: string) => string) {
  ...
  if (form.sourceType === "remote_managed") {
    if (!remoteWorkspaceRef && !repoUrl) {
      return t("workspace.remoteManagedRequiresRef");
    }
  } else if (!cwd && !repoUrl) {
    return t("workspace.requiresPathOrUrl");
  }

  if (cwd && (form.sourceType === "local_path" || form.sourceType === "non_git_path") && !isAbsolutePath(cwd)) {
    return t("workspace.localPathMustBeAbsolute");
  }
  ...
  if (repoUrl) {
    try {
      ...
      return t("workspace.repoUrlMustBeValid");
    }
  }
...
export function ProjectWorkspaceDetail() {
  ...
  useEffect(() => {
    ...
    setBreadcrumbs([
      ...
      { label: t("inbox.workspace"), href: `/projects/${canonicalProjectRef}/workspaces` },
  ...
  const updateWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : t("workspace.failedToSaveWorkspace"));
    },
  });
  ...
  const setPrimaryWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : t("workspace.failedToUpdateWorkspace"));
    },
  });
  ...
  const controlRuntimeServices = useMutation({
    ...
    onSuccess: (result, action) => {
      ...
      setRuntimeActionMessage(
        action === "stop"
          ? t("workspace.runtimeServicesStopped")
          : action === "restart"
            ? t("workspace.runtimeServicesRestarted")
            : t("workspace.runtimeServicesStarted"),
      );
    },
    onError: (error) => {
      setRuntimeActionMessage(null);
      setErrorMessage(error instanceof Error ? error.message : t("workspace.failedToControlRuntimeServices"));
    },
  });

  if (projectQuery.isLoading) return <p className="text-sm text-muted-foreground">{t("workspace.loadingWorkspace")}</p>;
  if (projectQuery.error) {
    return (
      <p className="text-sm text-destructive">
        {projectQuery.error instanceof Error ? projectQuery.error.message : t("workspace.failedToLoadWorkspace")}
      </p>
    );
  }
  if (!project || !workspace || !form || !initialState) {
    return <p className="text-sm text-muted-foreground">{t("workspace.notFoundForProject")}</p>;
  }
  ...
  const sourceTypeDescription = sourceTypeOption ? t(`workspace.${sourceTypeOption.descKey}`) : null;
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="inline-flex items-center rounded-full border border-border bg-background px-2.5 py-1 text-xs text-muted-foreground">
          {workspace.isPrimary ? t("workspace.primaryWorkspace") : t("workspace.secondaryWorkspace")}
        </div>
      </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between">
              <div className="space-y-2">
                ...
                <p className="max-w-2xl text-sm text-muted-foreground">
                  {t("workspace.projectWorkspaceDesc")}
                </p>
              </div>
              {!workspace.isPrimary ? (
                <Button
                ...
                <div className="inline-flex items-center gap-2 rounded-xl border border-emerald-500/25 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-700 dark:text-emerald-300 sm:max-w-sm">
                  ...
                  {t("workspace.isPrimaryWorkspace")}
                </div>
              )}
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <Field label={t("workspace.visibility")}>
                <select
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                ...
                >
                  {VISIBILITY_OPTIONS.map((option) => (
                    <option key={option.value} value={option.value}>{t(`workspace.${option.labelKey}`)}</option>
                  ))}
                </select>
              </Field>
            </div>
            ...
            <div className="mt-4 grid gap-4">
              <Field label={t("workspace.sourceType")} hint={sourceTypeDescription ?? undefined}>
                <select
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
              ...
              <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_auto]">
                <Field label={t("workspace.localPath")}>
                  <input
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <Field label={t("workspace.repoRef")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                    value={form.repoRef}
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("workspace.defaultRef")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                ...
                <Field label={t("workspace.sharedWorkspaceKey")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("workspace.remoteProvider")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                ...
                <Field label={t("workspace.remoteWorkspaceRef")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("workspace.setupCommand")} hint={t("workspace.setupCommandProjectHint")}>
                  <textarea
                ...
                <Field label={t("workspace.cleanupCommand")} hint={t("workspace.cleanupCommandProjectHint")}>
                  <textarea
                    className="min-h-28 w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
              ...
              <Field label={t("workspace.runtimeServicesJson")} hint={t("workspace.runtimeServicesJsonProjectHint")}>
                <textarea
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="space-y-1">
              <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("workspace.workspaceFacts")}</div>
            ...
            <DetailRow label={t("workspace.defaultRef")}>{workspace.defaultRef ?? t("common.none")}</DetailRow>
            <DetailRow label={t("workspace.updated")}>{new Date(workspace.updatedAt).toLocaleString()}</DetailRow>
          </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                  {t("workspace.sharedServicesDesc")}
                </p>
              </div>
            ...
            {workspace.runtimeServices && workspace.runtimeServices.length > 0 ? (
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div className="space-y-1">
                        ...
                        <div className="space-y-1 text-xs text-muted-foreground">
                          ...
                          <div>{service.command ?? t("workspace.noCommandRecorded")}</div>
              ...
              <p className="text-sm text-muted-foreground">
                {workspace.runtimeConfig?.workspaceRuntime
                  ? t("workspace.noRuntimeRunning")
                  : t("workspace.noRuntimeConfigured")}
              </p>
            )}
          </div>
        </div>
